...
4. **Cost accepted.** A hidden LLM per active task each round; the user judged it `< 2× tokens` since the main session does the heavy work. Daemon runs on a cheap model (`haiku`), read-only tools (`Read`/`Grep`/`Glob`), `maxTurns: 8`. This **bends** *pull-not-push* (background LLM work) and *near-zero diagnostic cost* — recorded as a deliberate, user-authorized exception.
