...
|---|---|---|
| `avoidance` | repeated long review visits, or ≥3 revisits over ≥1h without advancing | Decision is what's needed, not more "work" |
...
## v2.2: what's session-derived vs memory-derived (2026-05-31)

The user drew a sharper line through the signals than v2.1 had: **what you can read off a single session vs. what needs the whole memory to judge.**

- **Session-derived** (read off one session's JSONL): the matched `brief`, the `埋点`/telemetry (prompts/actions/dwell/age), and the behavioral `pattern` — including the `avoidance`/`决策回避` badge. These are observations of *this* session and stay session-local.
- **Memory-derived** (a judgment across your whole working context, so it reads the unioned memory corpus): **priority** and **ETA**. The reasoning: "is this worth re-engaging" and "how much will it cost me" are cross-cutting calls that a single session can't answer — they depend on everything you're carrying.

Two concrete changes (`priority.ts`, `server.ts`):

1. **Priority is now memory-led.** `ALIGN_WEIGHT` 6 → 12, so TF-IDF memory alignment is the primary driver; the pattern weights shrank to small nudges (avoidance 4 → 1, stalled 3 → 0.7, healthy −1 → −0.3) so a behavioral signal can no longer outrank what memory says matters. `avoidance` becomes a surfaced *observation*, not a ranking lever. Brief status (`deferred`/`done`) and explicit blockers still apply as overrides.
2. **ETA is now memory-derived, not transcript-derived.** This **reverses** the v2.1 rationale that had been written into `server.ts` ("memory drives priority; the transcript drives cost"). The old `estimateEtaMin` (last-turn char count ÷ 500 + staleness) was dropped. `estimateEtaFromMemory` scores the session against the memory corpus and maps alignment depth through a saturating `1 − exp(−cosine/τ)` curve to ~2–30 min: a topic your memory is deeply invested in costs more to reload + reply to thoughtfully; a peripheral one is cheap. `lastTurnChars` is still parsed but no longer consumed.

UI (`ui/console.ts`): the matched **brief is now shown inline at the top** of the chat panel — title = brief name, subtitle = the brief's `what` trimmed to one short line server-side (`briefSummary`, ≤140 chars, single line + ellipsis). No click-through to `/brief` needed to read the task; "brief 不要太长" is enforced by the trim.

What still holds: descriptive-not-judgmental telemetry, auditable one-line reasons (the memory `topTerms` are still surfaced), cross-vendor neutrality, single localhost surface. The one reversal is narrow and recorded: cost moved from transcript to memory at the user's explicit call.

## v2.3: brief/priority/ETA are produced by a per-session daemon (2026-05-31)

The user reframed the analyzer: instead of a static heuristic (or a one-shot summary call), **every task session created in our product gets a paired "daemon session"** — *a normal Claude session* that re-analyzes the task each round and emits the `brief`, `priority`, `etaMin`, and `reason`. The daemon is just filtered out of the product's listing. This **supersedes** the v2.2 heuristic for product-created sessions (the heuristic stays only as a **fallback** for historical / terminal-launched sessions that have no daemon).

Decisions (the five forks the user settled):

1. **Daemon owns brief + priority + ETA** (LLM judgment); the v2.2 memory-led heuristic + `estimateEtaFromMemory` become the **fallback** when a session has no daemon. `pattern` stays a session-derived *observation* (not the daemon's job). The analyzer is a **vendor seam** (`chat/analyzer/`, the daemon mirror of `SessionSource`): a session is analyzed by *its own vendor's* analyzer (`ClaudeAnalyzer` drives a Claude session; `CodexAnalyzer` is a stub that returns nothing until Codex exposes a resumable session). The contract/prompt lives inside each analyzer; the orchestrator only routes by vendor + caches, and the normalized `Analysis` stays vendor-neutral.
2. **Same cwd as the task.** The daemon shares the task's working dir so it can read the same repo + memory — which means it *cannot* be told apart by directory. A persistent **registry** (`~/.attend/daemons.json`, `core/daemon/registry.ts`) is the source of truth for which session ids are daemons; those ids are filtered out of every listing (`visibleSessions` in `server.ts`).
3. **Triggered on turn-end, not polled.** `ChatEngine.onTurnEnd` fires when a task turn completes; the server resumes the daemon (`query({resume})`), feeds it the condensed transcript, and the daemon replies with **one JSON object** (`{brief, priority, etaMin, reason}`), parsed string-aware in `core/daemon/parse.ts` (never fabricates — null on unparseable, same line as the Codex stub). Latest verdict is cached per task (`core/daemon/cache.ts`, `~/.attend/analysis.json`), overwritten each round (no lastTs gating). The console polls `/session/analysis` a few seconds after a turn to pick it up without a reload.
4. **Cost accepted.** A hidden LLM per active task each round; the user judged it `< 2× tokens` since the main session does the heavy work. The daemon no longer pins its own model/effort/tool profile; it follows the vendor's normal session defaults in that environment. This **bends** *pull-not-push* (background LLM work) and *near-zero diagnostic cost* — recorded as a deliberate, user-authorized exception.
5. **Auto-spawn on creation.** `/chat/new` and `/chat/fork` call `orchestrator.ensureDaemon`. Only product-created sessions get daemons ("历史的不管了").

UI (`ui/console.ts`): the `task`/`brief.md` concept is **gone entirely** — a tab's title is the daemon's `brief` (provisional first-prompt until the daemon replies), with **two subtitles** (`首` my first message, `新` my latest message) and **priority + ETA badges** so a tab is judgeable without opening it. `RawSession.lastPrompt` was added to carry the latest prompt. The legacy brief feed (`/briefs`, `/brief`) and its machinery (`core/brief.ts`, `rank.ts`, `telemetry.ts`'s `matchSessions`/`telemetryForBrief`, `core/spawn.ts`, `ui/feed.ts`, `ui/detail.ts`) were **deleted** at the user's call ("删掉"). `core/priority.ts` + `alignment` survive only as the no-daemon heuristic fallback.

## v2.4: "+ new" picks a provider, gated by local CLI detection (2026-05-31)

The "+ new" box was hardcoded to Claude (a leftover of the v2 in-browser-chat pivot). The user wanted it to **pick a provider, offering only CLIs actually installed on the machine**. Added `core/vendor/detect.ts` (`detectVendors`) — probes PATH (`where`/`which`, injectable for tests) for each vendor; lives in `core/vendor/` to keep vendor-locked execution behind that seam (invariant 4). The server detects once (cached 5 min — a CLI is rarely (un)installed mid-run) and passes `vendors` into `ConsoleView`; the picker renders a `<select>` where unavailable vendors are `disabled` ("未检测到") and Codex is tagged "终端".

Routing respects the in-browser-chat-is-Claude-only constraint: **Claude → existing `/chat/new`** (in-browser, gets a daemon); **Codex → `POST /launch?action=new`** (terminal launcher — no streaming SDK), surfacing in the list on the next scan. No new endpoint; reuses the v1.2 launcher.

## v2.6: tab status — "your turn" vs. "you looked" (2026-06-03)

The sidebar status dot conflated two different things into "green vs. gray": a turn ended (green) and you'd opened the tab (gray). Opening a session called `markRead`, so a triage sweep — open several finished sessions, judge which to push, reply to one — **silently cleared the green on every tab you merely glanced at**. You then had to reopen each to rediscover which still owed a reply. Viewing was destroying the very signal the dot existed to carry.

Fix: split the dot onto two orthogonal axes — *whose turn* (objective: a turn ended and you haven't replied) and *have you seen it* (subjective) — and add a third visual state between bright-green and gray:

- **`unread`** (solid green) — a turn ended, unopened → "new, look at me"
- **`seen`** (hollow green ring) — opened, not yet replied → "still owe a reply"
- **`read`** (gray) — you've replied / nothing pending

The rule that resolves the original bug: **viewing only demotes `unread`→`seen` (stays green); only *replying* clears green→`read`.** So opening tabs to triage leaves them in the pool as hollow-green; the pool drains only as you actually advance each conversation. A subsequent turn-end re-arms the dot (`markUnread`/`markSeen(force)`), so a replied-to session that the agent answers again correctly returns to "your turn." Replying is detected at the two advance points — `dispatchSend` (typed + queued sends) and the question-answer handler. State stays client-local and ephemeral (same as the prior `unread`), consistent with the dot being a transient attention hint, not brief-state (invariant 1). Animation unchanged: `generating` still pulses.

**Manual dismiss (`toggleStatus`).** Reply isn't the only way a session leaves "your turn" — some resolve by decision ("parked," "not actionable," "read it, dropping it"). Without an explicit exit those sit hollow-green forever and the pool never fully drains. So the dot is **clickable** (`stopPropagation` keeps it from opening the row): click a green dot → `read` (dismiss); click a gray dot → `seen` (re-flag for follow-up, email-style "mark unread"). It's a reversible toggle — one-way dismiss is unrecoverable on a 0.5em target — and no-op mid-turn (the pulse isn't a toggleable state). A later turn-end still re-arms green regardless, since that's a genuinely new event. This completes the state machine: **reply or dismiss** both clear green; only an agent turn brings it back.

## v2.7: chat duplication on mid-turn reopen (2026-06-03)

Reopening (or reloading) a session **while its turn is still live** rendered duplicated, out-of-order assistant blocks ("重复和错乱"). Cause: `select()` loads JSONL/rollout history *and* re-attaches the live stream, whose `subscribe()` **replays the run's buffered events** (it does so whenever `turnActive`). Both sources carry the in-progress turn, so every persisted assistant block was rendered twice — once from history, once from the replay — landing after the tools (hence the disorder). Tools were already safe (deduped by `tool_use` id in `toolEls`); the old guard only protected the *last* assistant bubble via a prefix-strip (`pendingAssistantDedupe`), so any earlier block duplicated.

Fix is client-side and vendor-neutral: a **content-keyed catch-up dedup**. After history renders, `primeCatchup()` counts every assistant block already on screen. While the replay window is open — from history-load until the `sync` marker the engine sends right after the buffer catch-up — `consumeCatchup(text)` drops any `assistant_text` whose text is still accounted for in that count; the first block *not* in the snapshot is genuinely new content and ends the window (so all live output after it renders normally). `sync`, `beginTurn`, `error`, and `select` reset the window. This works because each replayed `assistant_text` is one atomic block matching one history bubble 1:1 (the SDK emits complete blocks, not deltas), and tools continue to dedup by id. Verified by driving a real mid-turn reopen via Playwright: the first assistant block, present in both history and the replay, now renders exactly once. State is ephemeral/client-only (invariant 1). The server buffer (which spans the whole run) was left as-is; the client dedup makes the overlap harmless regardless of how much is replayed.

**Sibling fix — duplicated *user* messages after stop-then-resend.** A second report: send msg 1 → **Stop** → send msg 2 → after the turn, msgs 1 and 2 reappear at the bottom. Different mechanism, same family. `localPendingMsgs` holds optimistically-rendered sends so a reopen before the JSONL flush doesn't lose them; `transcriptPendingTail()` re-appends only the ones not yet in the transcript. It used to match the pending list as a **contiguous suffix** of the transcript's user messages — but an aborted send that never persisted (msg 1, Stopped immediately) punches a hole, so `[msg1, msg2]` is no longer a suffix of `[…, msg2]`, the match fails, and *both* get re-appended on the next `select()` re-render. Fixed by matching each pending message as an in-order **subsequence** and dropping everything up to and including the last *confirmed* one: if a later pending msg reached the transcript, an earlier missing one was aborted and never will, so it's settled too — not still-pending. Verified against the bug case + lagging/partial-flush cases (send-before-flush still re-appends correctly).

**Third fix — slash-command turns rendered as the wrong role / out of order.** A third report (a `/check-logs` session): the top half of the chat was assistant content shown as *the user's* message, and the user's own command landed at the bottom, after the reply. Root cause was entirely in `readClaudeTranscript` (Claude stores a typed slash command as two turns): the human turn is `<command-message>…</command-message><command-name>/x</command-name><command-args>…</command-args>` — which the old `text.startsWith("<")` drop **discarded** — immediately followed by a separate **`isMeta: true`** turn carrying the command's *expanded template* (a multi-KB skill doc), which was **kept** and rendered as the user's message. So the real command vanished and the template took its place; and because the typed command never made it into the parsed transcript, `transcriptPendingTail` (when the command was sent through attend) couldn't reconcile it and re-appended it at the bottom — the "out of order" half. Fix: skip `isMeta` turns entirely, and reconstruct the typed command (`/x args` from `<command-name>`+`<command-args>`) as the user message instead of dropping it. With the real command back in the transcript, the pending-tail reconciliation also matches and stops re-appending it. Other `<…>` synthetic turns (`<ide_opened_file>`, system reminders) still stay hidden. Verified on the real session file (command now leads, in order; template gone) + a unit test (`test/transcript.test.ts`).

## v2.8: daemon-drafted "next step" beside the composer todo (2026-07-15)

The composer rail already carried the human's own `shortcuts`/`notes`/`todo` (all persisted server-side in `VaultUiStateStore`). The user wanted **one more slot to the right of `todo`: a sentence the daemon writes** — for turns where the next user message is *obviously* a mechanical push ("continue", "apply it and run the tests"), draft it so a single click drops it into the composer, no typing. And when drafting it, feed the daemon the human's own shortcuts/notes/todo as context.

Four forks the user settled:

1. **New field, not merged with `avoidancePrompt`.** The daemon's `Analysis` gains `nextStep?: string | null` (separate from the existing avoidance draft). The two are semantically opposite — `avoidancePrompt` is "you're re-opening this without progress, here's a friction-lowering nudge" (gated behind the `avoidance` pattern, one-shot); `nextStep` is "the next move is obvious, here's the message you'd have typed." Sharing one field would tangle avoidance's decision-回避 handling with a routine continuation.
2. **Fill, not send.** Clicking the pill fills the composer (`input.value` + focus/select, the `draftAvoidancePrompt` pattern) and waits for the human to press Enter — it never auto-sends. Sending stays an explicit human action (invariant 2); the draft only removes the typing.
3. **Always the *most likely* next message, not only the obvious one (user redirect).** First cut gated `nextStep` to mechanical states (`continue_ready`/`followup_suggested`) and left it empty on any decision — "never draft a decision for the human." The user rejected that as too narrow: *"当然要的不只是明显推进, 就是给我生成最可能的推进"* — draft the single most probable next message **every** turn, including when a decision is pending (pick the option the transcript makes most likely and phrase it as the human's reply). This deliberately relaxes the strict reading of invariant 3, and the relaxation is safe because it's **prediction, not decision, and fill-not-send**: the pill only pre-fills the composer, so the human still confirms or edits before anything is sent (invariant 2 intact). `nextStep` rides the existing turn-end verdict (no extra LLM call). It's an empty string only when nothing can be inferred (task done / no transcript yet); `parseAnalysis` maps `""` → `null`, and the server nulls it while a turn is generating, so the pill shows on essentially every human turn but never mid-generation.
4. **The human's own context feeds the daemon — read server-side, no new client→server channel.** `shortcuts`/`notes`/`todo` already live in `VaultUiStateStore`, so `condenseUiContext()` (in `chat/analyzer/contract.ts`) formats them into a bounded block that the server (`daemonUiContext(sid)`, keyed by provider session id — the same key the console uses for notes/todos) threads through `analyzeTask` → the vendor analyzer → `requestPrompt`. **This same block is also fed to `avoidancePromptRequest`** (the user asked: avoidance should get this data too), so both drafts can point at the user's real todos instead of generic phrasing. The daemon's clean "reads transcript + shared cwd" model is preserved — the UI context is just extra prompt input, not a new dependency.

UI: one pill `#railNextStep` (`ui/console.ts`) in the composer rail, right of `todo` — renders the draft text (truncated, accent-tinted), hidden unless `nextStep` is present and the session isn't generating; a click calls `fillNextStep()`. It picks up the verdict through the existing `/session/analysis` poll + the initial session-view field, the same path as the other daemon signals.
